...
public class UserEndpointsLogicTests
{
    ...
    {
        // Arrange
        _userServiceMock.Setup(x => x.GetUsersAsync(userQuery)).ReturnsAsync(new CompleteUserDTO(0, []));
    ...
    {
        // Arrange
        _userServiceMock.Setup(x => x.GetUsersAsync(It.IsAny<UserQuery>())).ReturnsAsync(new CompleteUserDTO(userDTOs.Count, userDTOs));
        ...
        // Assert    
        var okResult = Assert.IsType<Microsoft.AspNetCore.Http.HttpResults.Ok<CompleteUserDTO>>(result);
